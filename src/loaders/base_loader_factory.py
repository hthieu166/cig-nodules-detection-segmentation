...
class BaseDataLoaderFactory():
    ...
    def get_data_split(self):
        ...
        self.ld_dict['train_val'] = {k.split('/')[1]:{} 
                    for k in self.dataset_params['datalst_pth'].keys() if 'train_val' in k}
        self.ld_dict['test'] = {k.split('/')[1]:{} 
                    for k in self.dataset_params['datalst_pth'].keys() if 'test' in k}            
        # ipdb.set_trace()
    ...
    def get_transform(self, transform_config):
        ...
        trsf_dict = {'transforms' : all_transforms}
        composed_transforms = self.data_augment_fact.generate(
            "Compose",trsf_dict)
